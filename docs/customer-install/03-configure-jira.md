...
real values against your project and put them in `agent/k8s/triage-config.yaml`
(copy from `agent/k8s/triage-config.example.yaml`).
...
`agent/k8s/triage-listener.yaml`. This stops anyone-who-can-edit-labels from
...
**Prereq:** set `automation-shared-secret` in `agent/k8s/triage-secrets.yaml`
...
**Prereq:** set `webhook-hmac-secret` in `agent/k8s/triage-secrets.yaml`
